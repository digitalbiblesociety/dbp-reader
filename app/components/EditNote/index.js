...
class EditNote extends React.PureComponent {
  ...
  constructor(props) {
    super(props);
    // Tags field needs to be parsed to find the title
    const hasTitle =
  ...
  handleTextareaChange = (e) => {
    ...
    this.setState(
      (cs) => (cs.readTheMessage ? { textarea: e.target.value } : { textarea: e.target.value, readTheMessage: true })
    );
  };
  ...
  handleNoteTitleChange = (e) => {
    ...
    this.setState(
      (cs) => (cs.readTheMessage ? { titleText: e.target.value } : { titleText: e.target.value, readTheMessage: true })
    );
  };
  ...
  handleSave = (e) => {
    ...
    // If there wasn't a message entered for the note then display an error message
    if (!textarea) {
    ...
    if (note.get('notes') === textarea && prevTitle === titleText) {
      ...
      // If the note already existed then it needs to be updated
    ...
    return (
      <section className="edit-notes">
        ...
        <div className={`verse-dropdown${isVerseTextVisible ? ' open' : ''}`}>
          ...
          <span className="version-dropdown">{note.get('bible_id') || activeTextId}</span>
        </div>
        {isVerseTextVisible && <NoteVerseText loading={loadingChapterForNote} notePassage={notePassage} />}
        <textarea
        ...
        <div className={'delete-note-section'}>
          <span className="delete-button" role="button" tabIndex={0} onClick={() => this.deleteNote()}>
            <SvgWrapper className={'icon'} svgid={'delete'} />
            Delete Note
          </span>
          {savedTheNote && !errorSavingNote ? (
            <span className="saved-note" role="button" tabIndex={0} onClick={this.handleSave}>
              Saved
            </span>
          ) : (
            <span
            ...
            >
              {savingNote && !errorSavingNote ? 'Updating...' : `${errorSavingNote ? 'Failed' : 'Save'}`}
            </span>
          )}
        </div>
        {popupOpen ? (
          <PopupMessage
            message={errorSavingNote && notesErrorMessage ? notesErrorMessage : 'The notes field cannot be empty.'}
            x={coords.x}
            y={coords.y}
          />
        ) : null}
