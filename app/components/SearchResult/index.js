...
function SearchResult({ result: r, bibleId }) {
	...
	return (
		<div key={`${r.get('book_id')}${r.get('chapter')}${r.get('verse_start')}`} className={'single-result'}>
			<h4><Link to={`/${bibleId}/${r.get('book_id')}/${r.get('chapter')}/${r.get('verse_start')}`}>{`${r.get('chapter_alt')}:${r.get('verse_start_alt')}`}</Link></h4>
