...
class MyNotes extends React.PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    // Makes sure the pagination component stays up to date and that
    // the right data is being displayed
    if (
      nextProps.sectionType === 'notes' &&
      (this.props.pageSize !== nextProps.pageSize || this.props.activePage !== nextProps.activePage)
    ) {
      this.props.getNotes({
  ...
  getFormattedNoteDate = (timestamp) => {
    ...
    // Returns a date formatted as m.d.yy
  ...
  getFilteredPageList = (pageData) => {
    ...
    // Each listed key is prioritized based on its position in the list
  ...
  getHighlightReference = (highlight) => {
    if (highlight.reference) {
      return highlight.reference;
    }
    // If there wasn't a reference already part of the highlight then one needs to be made
    return `${highlight.book_id} - ${highlight.chapter}:${
      highlight.verse_start === highlight.verse_end || !highlight.verse_end
    ...
    return (
      <div className="list-sections">
                ...
                <div key={listItem.id} id={listItem.id} className={'highlight-item'}>
                  <div role="button" tabIndex={0} onClick={() => this.handleClick(listItem)} className="list-item">
                    <div className="title-text">
                      <h4 className="title">
                        <span className="date">{this.getFormattedNoteDate(listItem.created_at)}</span> |{' '}
        ...
        <div className="pagination">
          <Pagination onChangePage={this.handlePageClick} activePage={this.activepage} totalPages={this.totalpages} />
          <PageSizeSelector
