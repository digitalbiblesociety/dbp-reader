...
class VideoPlayer extends React.PureComponent {
  ...
  componentDidMount() {
    this.getHls();
    this.videoRef.addEventListener('webkitendfullscreen', this.webkitendfullscreen, false);
    Router.router.events.on('routeChangeStart', this.handleRouteChange);
  }

  componentWillReceiveProps(nextProps) {
    const { fileset, videoAssetId } = nextProps;

    if (
      nextProps.bookId !== this.props.bookId ||
      nextProps.chapter !== this.props.chapter ||
      (nextProps.fileset && this.props.fileset && !isEqual(nextProps.fileset, this.props.fileset))
    ) {
    ...
    } else if (nextProps.hasVideo !== this.props.hasVideo && nextProps.hasVideo) {
      this.getVideos({
        filesetId: fileset ? fileset.id : '',
        bookId: nextProps.bookId || '',
        chapter: nextProps.chapter,
  ...
  componentWillUnmount() {
    // Cleans up all the event listeners to prevent memory leaks
    if (this.hls && this.hls.media) {
      this.hls.media.removeEventListener('timeupdate', this.timeUpdateEventListener);
    ...
    if (this.videoRef) {
      this.videoRef.removeEventListener('timeupdate', this.timeUpdateEventListener);
      ...
      this.videoRef.removeEventListener('webkitendfullscreen', this.webkitendfullscreen);
  ...
  getHls = async () => {
    // Dynamically requiring hls to reduce the bundle size by about 500kb
    const hls = await import('hls.js');
    const { fileset, bookId, chapter, videoAssetId } = this.props;
  ...
  getVideos = async ({ filesetId, bookId, chapter, videoAssetId }) => {
    if (!filesetId) return;
    const requestUrl = `${process.env.BASE_API_ROUTE}/bibles/filesets/${filesetId}?key=${
      process.env.DBP_API_KEY
    }&v=4&type=video_stream&asset_id=${videoAssetId}&book_id=${bookId}`;
    ...
    try {
      ...
      if (response.data) {
        ...
        // Change the video response into the object I want to work with
        const videos = response.data.map((video, index) => ({
          title: `${video.book_name} ${video.chapter_start}:${video.verse_start}-${video.verse_end}`,
          ...
          reference: `${video.chapter_start}:${
            video.verse_end ? `${video.verse_start}-${video.verse_end}` : video.verse_start
          }`,
          thumbnail: `${'mark' || video.book_name}_${video.book_id.toLowerCase()}_${index}.jpg`,
        }));
        const playlist = videos.filter((video) => video.bookId === bookId && video.chapterStart === chapter);
        this.setState({
          videos,
    ...
    this.setState(
      (state) => ({
        playlist: state.videos
          .filter((v) => v.id !== video.id && v.bookId === bookId && v.chapterStart === chapter)
  ...
  initVideoStream = ({ thumbnailClick }) => {
    ...
    // Fallback for IE and Safari (Safari has hls streaming built in)
    if (!hlsSupported) {
      if (this.videoRef.canPlayType('application/vnd.apple.mpegurl')) {
        this.videoRef.src = `${currentVideo.source}?key=${process.env.DBP_API_KEY}&v=4&asset_id=${
          this.props.videoAssetId
        }`;
        this.videoRef.addEventListener('timeupdate', this.timeUpdateEventListener);
      ...
      try {
        ...
        if (this.videoRef) {
          ...
          if (currentVideo.source) {
            ...
            this.hls.loadSource(
              `${currentVideo.source}?key=${process.env.DBP_API_KEY}&v=4&asset_id=${this.props.videoAssetId}`
            );
  ...
  render() {
    const { playlist, volume, paused, elipsisOpen, currentVideo, currentTime, bufferLength } = this.state;
    const { hasVideo, playerOpen, fileset, books, bookId, chapter, textId, text } = this.props;
    ...
    return [
      <div
        ...
        className={playerOpen ? 'video-player-container active' : 'video-player-container'}
      >
        <div className={'video-player'}>
          ...
          <video
            ...
            poster={`${process.env.CDN_STATIC_FILES}/${currentVideo.thumbnail || 'mark_mrk_0.jpg'}`}
          />
          <VideoProgressBar
            paused={paused}
      ...
      <div key={'black-bar-key'} onClick={this.openPlayer} className={playerOpen ? 'black-bar closed' : 'black-bar'}>
