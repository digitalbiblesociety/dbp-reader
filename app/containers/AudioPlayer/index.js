...
export class AudioPlayer extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		// If the user changed chapters or audio types then the next audio needs to be fetched
		if (
		...
		// If the source changed then the interface needs to be updated
		...
		if (nextProps.autoPlay) {
			// Checks for iOS devices since they have a different set of apis
			if (
				navigator &&
				navigator.userAgent &&
				/iPhone|iPod|iPad/i.test(navigator.userAgent)
	...
	componentWillUnmount() {
		// Removing all the event listeners to prevent memory leaks
		if (
			navigator &&
			navigator.userAgent &&
			/iPhone|iPod|iPad/i.test(navigator.userAgent)
	...
	endedEventListener = () => {
		// Check if there is another track within this chapter
