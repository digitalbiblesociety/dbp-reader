...
/**
 * Applies all the user notes to the formatted text before the text is inserted into the dom
 * @param {String} source The source text formatted as html
 * @param {Array} notes All the user notes for the current chapter
 */
const applyNotes = (source, notes) => {
  ...
  notes.forEach((note) => {
    if (
      ...
      const verseElement = verses.find((v) => getVerseNum(v) === note.verse_start);
...
/**
 * Applies all the user highlights to the formatted text before the text is inserted into the dom
 * @param {String} source The source text formatted as html
 * @param {Array} highlights All the user highlights for the current chapter
 */
const applyWholeVerseHighlights = (source, highlights) => {
  ...
  highlights.forEach((highlight) => {
    const verses = [
      ...xmlDoc.getElementsByClassName(`${highlight.book_id}${highlight.chapter}_${highlight.verse_start}`),
    ];
    // Todo: If the verse is the 1st verse then there will not be an element for it
    // Create the element and add it into the front of the array of children
    ...
    if (highlight.verse_start === getVerseHighlightNum(verses[0])) {
      verses.forEach((verseElement) => {
        ...
        verseElement.setAttribute(
          'style',
          `background:linear-gradient(${highlight.highlighted_color ? highlight.highlighted_color : 'inherit'},${
            highlight.highlighted_color ? highlight.highlighted_color : 'inherit'
          })`
        );
      });
    }
...
/**
 * Applies all the user bookmarks to the formatted text before the text is inserted into the dom
 * @param {String} source The source text formatted as html
 * @param {Array} bookmarks All the user bookmarks for the current chapter
 */
const applyBookmarks = (source, bookmarks) => {
  ...
  bookmarks.forEach((bookmark) => {
    if (
      ...
      const verseElement = verses.find((v) => getVerseNum(v) === bookmark.verse);
      const svg = xmlDoc.createElement('svg');
      const use = xmlDoc.createElement('use');
      // Can't set the event handlers here because this "dom" is removed
      use.setAttribute('xmlns:xlink', 'http://www.w3.org/1999/xlink');
...
/**
 * Gets the verse number from the class attribute on highlighted verse nodes
 * @param {Object} verse Dom node
 */
...
/**
 * Gets the verse number from the class attribute on non-highlighted verse nodes
 * @param {Object} verse Dom node
 */
