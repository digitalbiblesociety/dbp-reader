...
import { CLOSE_VIDEO_PLAYER, OPEN_VIDEO_PLAYER, SET_HAS_VIDEO } from '../VideoPlayer/constants';
import {
  ACTIVE_TEXT_ID,
  CHANGING_VERSION,
  LOAD_AUDIO,
...
function homePageReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case RESET_BOOKMARK_STATE:
      return state.set('addBookmarkFailure', false).set('addBookmarkSuccess', false);
    case LOAD_AUDIO:
    ...
    case SET_ACTIVE_BOOK_NAME:
      return state.set('activeBookId', action.id).set('activeBookName', action.book);
      ...
      return state
        ...
        .setIn(
          ...
          action.hasFormattedText &&
            (action.formattedText.includes('class="ft"') || action.formattedText.includes('class="xt"'))
        )
        .setIn(
          ['userSettings', 'toggleOptions', 'redLetter', 'available'],
          action.hasFormattedText &&
            (action.formattedText.includes('class="wj"') || action.formattedText.includes("class='wj'"))
        )
    ...
    case 'loadcopyright':
      return state.set('loadingCopyright', false).set('copyrights', action.copyrights);
    case 'GET_INITIAL_ROUTE_STATE_HOMEPAGE':
      // This case merges all of the state passed through in the action
