...
export class Verses extends React.PureComponent {
  ...
  getLastVerse = (e) => {
    ...
    const { formattedSource, userSettings, activeChapter, activeBookId } = this.props;
    ...
    if (lastVerse.openMenu) {
      // Need to open the menu after the rest of the state has been set
      // That is why a callback is used here
      this.setState(
        {
  ...
  setActiveNotesViewMethod = (view) => this.props.dispatch(setActiveChild(view));

  setActiveNote = ({ coords, existingNote, bookmark }) => {
    const { userAuthenticated, userId, activeBookId, activeChapter, activeTextId } = this.props;
  ...
  setFootnotes = (footnotes) => this.setState({ footnotes, footnoteState: false });
  ...
  handleNoteClick = (noteIndex, clickedBookmark) => {
    ...
    // Figure out which side menu to open
    if (!notesActive) {
  ...
  handleAddBookmark = () => {
    const { activeBookId, userId, userAuthenticated, activeChapter, activeTextId, activeBookName } = this.props;
    const { firstVerse, lastVerse } = this.state;
    ...
    if (userAuthenticated && userId) {
      this.props.dispatch(
        addBookmark({
          ...
          user_id: userId,
          bible_id: activeTextId,
          reference: getReference(verseStart || verseEnd, verseEnd, activeBookName, activeChapter),
  ...
  deleteHighlights = (highlightObject, highlights) => {
    const space = highlightObject.highlightStart + highlightObject.highlightedWords;
    const highsToDelete = highlights
      .filter(
        (high) =>
          high.verse_start === parseInt(highlightObject.verseStart, 10) &&
          (high.highlight_start <= space &&
            high.highlight_start + high.highlighted_words >= highlightObject.highlightStart)
      )
      .reduce((a, h) => [...a, h.id], []);
  ...
  selectedWholeVerse = (verse, isPlain, clientX, clientY, userSelectedText) => {
    if (typeof this.window !== 'undefined') {
      const rightEdge = this.window.innerWidth < 500 ? this.window.innerWidth - 295 : this.window.innerWidth - 250;
      const bottomEdge = this.window.innerHeight < 900 ? this.window.innerHeight - 317 : this.window.innerHeight - 297;
      ...
      if (isPlain) {
        this.setState((currentState) => ({
          coords: { x, y },
          wholeVerseIsSelected: !(currentState.wholeVerseIsSelected && currentState.activeVerseInfo.verse === verse),
  ...
  openContextMenu = (e) => {
    if (typeof this.window !== 'undefined') {
      ...
      setTimeout(() => {
        if (typeof this.window !== 'undefined' && !this.window.getSelection().toString()) {
          this.closeContextMenu();
        } else {
          this.setState({
            coords: { x, y },
  ...
  dispatchAddHighlight = (props) => this.props.dispatch(addHighlightAction(props));
    ...
    return (
      ...
      >
        {!formattedSource.main &&
          !text.length && <AudioOnlyMessage key={'no_text'} book={activeBookName} chapter={activeChapter} />}
        {(formattedSource.main && !readersMode && !oneVersePerLine) ||
        text.length === 0 ||
        (!readersMode && !oneVersePerLine) ? null : (
          <div className="active-chapter-title">
            <h1 className="active-chapter-title">{chapterAlt || activeChapter}</h1>
          </div>
        )}
        {formattedSource.main &&
          domMethodsAvailable &&
        ...
        {(!formattedSource.main || readersMode || oneVersePerLine || !domMethodsAvailable) &&
          !!text.length && (
            <PlainText
              initialText={text}
              highlights={highlights}
        ...
        {verseNumber && (
          <ReadFullChapter activeTextId={activeTextId} activeBookId={activeBookId} activeChapter={activeChapter} />
        )}
        <Information />
      </main>
    );
