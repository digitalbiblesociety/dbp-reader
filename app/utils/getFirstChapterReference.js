 ...
 * @param {Bool} hasVideo
 * @param {Array} bookMetaResponse
 * @param {Array} bookMetaData
 * @param {String} audioParam
 */
const getFirstChapterReference = (filesets, hasVideo, bookMetaResponse, bookMetaData, audioParam) => {
  const hasOtAudio = filesets.some(
    (fileset) =>
      (fileset.size === 'OT' || fileset.size === 'OTP') && (fileset.type === 'audio' || fileset.type === 'audio_drama')
  );
  const hasNtAudio = filesets.some(
    (fileset) =>
      (fileset.size === 'NT' || fileset.size === 'NTP') && (fileset.type === 'audio' || fileset.type === 'audio_drama')
  );
  const hasOtText = filesets.some(
    (fileset) =>
      (fileset.size === 'OT' || fileset.size === 'OTP' || fileset.size === 'C') &&
  ...
  if (hasVideo) {
    ...
    const videoBooks = bookMetaResponse.find((filesetObject) => filesetObject[video.id]);

    reference = `${videoBooks[video.id][0].book_id}/${
      videoBooks[video.id][0].chapters[0] >= 0 ? videoBooks[video.id][0].chapters[0] : '1'
    }`;
  } else if (hasNtText && hasNtAudio) {
    const audioDrama = filesets.find(
      (fileset) => (fileset.size === 'NT' || fileset.size === 'NTP') && fileset.type === 'audio_drama'
    );
    const audio = filesets.find(
      (fileset) => (fileset.size === 'NT' || fileset.size === 'NTP') && fileset.type === 'audio'
    );
    const textFormat = filesets.find(
      (fileset) => (fileset.size === 'NT' || fileset.size === 'C') && fileset.type === 'text_format'
    );
    const textPlain = filesets.find(
      (fileset) => (fileset.size === 'NT' || fileset.size === 'C') && fileset.type === 'text_plain'
    );

    const audioBooks = bookMetaResponse.find(
      (filesetObject) => (audioDrama ? filesetObject[audioDrama.id] : filesetObject[audio.id])
    );
    const textBooks = bookMetaResponse.find(
      (filesetObject) => (textFormat ? filesetObject[textFormat.id] : filesetObject[textPlain.id])
    );
    ...
    const audioDrama = filesets.find(
      (fileset) => (fileset.size === 'OT' || fileset.size === 'OTP') && fileset.type === 'audio_drama'
    );
    const audio = filesets.find(
      (fileset) => (fileset.size === 'OT' || fileset.size === 'OTP') && fileset.type === 'audio'
    );
    const textFormat = filesets.find(
      (fileset) =>
        (fileset.size === 'OT' || fileset.size === 'OTP' || fileset.size === 'C') && fileset.type === 'text_format'
    );
    const textPlain = filesets.find(
      (fileset) =>
        (fileset.size === 'OT' || fileset.size === 'OTP' || fileset.size === 'C') && fileset.type === 'text_plain'
    );
    ...
    const audioBooks = bookMetaResponse.find((filesetObject) => filesetObject[audioId]);

    reference = `${audioBooks[audioId][0].book_id}/${
      audioBooks[audioId][0].chapters[0] >= 0 ? audioBooks[audioId][0].chapters[0] : '1'
    }`;
  } else if (hasOtAudio) {
    ...
    const textBooks = bookMetaResponse.find((filesetObject) => filesetObject[textId]);

    // Gets first book id and first chapter number from that book
    reference = `${textBooks[textId][0].book_id}/${
      textBooks[textId][0].chapters[0] >= 0 ? textBooks[textId][0].chapters[0] : '1'
    }`;
  } else if (hasOtText) {
    ...
    reference = `${bookMetaData[0].book_id}/${bookMetaData[0].chapters[0] >= 0 ? bookMetaData[0].chapters[0] : '1'}`;
  }
