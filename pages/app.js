...
import isoOneToThree from '../app/utils/isoOneToThree.json';
...
class AppContainer extends React.Component {
  ...
  componentDidMount() {
    if (this.props.setRedirectCookieToFalse) {
      document.cookie = 'bible_is_redirected=false;path=/';
    }
    if (
...
AppContainer.getInitialProps = async (context) => {
  ...
  let bibleId = bibleIdParam || 'ENGESV';
  ...
  let audioType = '';
  let initialIso = 'eng';
  let setRedirectCookieToFalse = false;
  let isoCodesDidNotMatch = false;
      ...
      if (req && req.headers && req.headers['accept-language']) {
        ...
        if (languages) {
          const langArray = languages
            ...
            .filter((lang) => !!lang);
          console.log('lang array server init', langArray);
          console.log('first language', langArray[0]);
          console.log('----------------------------------------------------');
          initialIso = langArray[0] || 'eng';
        }
      }
      setRedirectCookieToFalse = true;
    }
  ...
  let bible = singleBibleJson.data || {};

  if (bible.iso !== initialIso && wasRedirected && serverRes) {
    isoCodesDidNotMatch = true;
    const biblesData = await cachedFetch(
      `${process.env.BASE_API_ROUTE}/bibles?key=${
        process.env.DBP_API_KEY
      }&v=4&asset_id=${process.env.DBP_BUCKET_ID},dbp-vid`,
    ).catch((err) => {
      if (process.env.NODE_ENV === 'development') {
        console.error('Error in get initial props bible for language: ', err); // eslint-disable-line no-console
      }
      return { data: [] };
    });
    ...
    if (biblesInLanguage[0]) {
      ...
      // Get active bible data
      const bibleRes = await cachedFetch(requestUrl).catch((e) => {
        if (process.env.NODE_ENV === 'development') {
          console.error(
  ...
  if (bookMetaData && bookMetaData.length) {
    ...
    if (isoCodesDidNotMatch && serverRes && wasRedirected) {
      console.log('redirect to new base route');
      ...
      serverRes.end();
      console.log('after server res.end()');
    }
