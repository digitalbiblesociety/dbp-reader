...
class AppContainer extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.formattedText !== this.props.formattedText) {
			// If formatted text has words of Jesus (red text) then it will contain the class wj
			// it could be in either single or double quotes
			const redLetter =
			...
			// If there are footnotes then the text will either contain one of the classes [ft, xt]
			this.props.dispatch({
				type: 'GET_INITIAL_ROUTE_STATE_SETTINGS',
				redLetter,
				crossReferences:
...
AppContainer.getInitialProps = async (context) => {
	...
	const dbpDefaultBucket =
		bible && bible.filesets && bible.filesets[process.env.DBP_BUCKET_ID];
	const videoAssetId =
		!dbpDefaultBucket ||
		!bible.filesets[process.env.DBP_BUCKET_ID].find(
			(file) => file.type === 'video_stream',
		)
			? 'dbp-vid'
